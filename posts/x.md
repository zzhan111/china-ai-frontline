...
- 龙虾课生态：39.8 元→18999 元，证券时报调查确认全职妈妈是主力购买人群
...
龙虾课从 39.8 到 18999 元，买的人里最多的是全职妈妈。
...
· 引流（99）：Agent 全家桶安装，主流 Agent 框架一键部署
...
## post-2026-06-06-001：主动型 Agent——当AI开始主动打量你
...
桌面 Agent 正在从「你说我做」走向「我不说你也能做」，时机判断是最后一道坎。

### 近似实现 / 需要调查

调研发现桌面 agent 赛道已形成四层能力栈：视觉感知（CogAgent/OmniParser）→ 时机判断（当前空白）→ 专业知识（RAG）→ 工具执行（MCP/function calling）。主动型 Agent 的核心差异化在第二层（时机判断），目前项目集中在第一层和第四层。与 inbox 原想法的差异：原以为工具调用是核心瓶颈，调研后发现「什么时候该开口」才是真正的无人区。

### X thread 草稿

**1/**
今天测了一个桌面 agent，它开着摄像头看我工作，然后主动问我：「你在调那个 API 吧？我这里有份文档，需要我帮你查吗？」

**2/**
第一反应是不适。缓过来之后，我开始琢磨另一件事：如果它能识别我此时此刻在做什么，在合适的时机用它的专业知识主动插话。它已经不是 chatbot 了，这是主动型 Agent。

**3/**
主动型 Agent 拆成四层：
· 视觉感知（屏幕 + 摄像头）
· 时机判断（什么时候该开口，什么时候该闭嘴）
· 专业知识（开口了得说对）
· 工具执行（不止聊天，真能操作）
...
2024 年 UFO、Operator 等项目把「协同执行」（你说我做）做扎实了。现在的重点在第三阶段：什么时候该主动开口。

**5/**
几个关键项目：
· Microsoft UFO：最成熟的 Windows 桌面 agent，开源
· Claude Computer Use：Anthropic 首推「AI 操控电脑」
· OmniParser：把截图解析成 UI 元数据，主动 agent 的「眼睛」
· OSWorld：首个真实 OS 环境的 agent 基准测试

**6/**
坦率讲，四层里「时机判断」是最难的一层。视觉感知有 CogAgent/ScreenAgent 撑着，工具调用有 MCP/function calling，专业知识有 RAG。但「什么时候该打断用户」，目前没人做得特别好。

**7/**
我猜下一步会有人把 OSWorld 这类 benchmark 里的「主动性」单独拉一个维度出来评测。现在所有 desktop agent 评测都在测「执行准确率」，没人测「什么时候该主动行动」。

那个会主动打量你的 agent，只是第一步。
...
应用项：
- 删 em dash ×2（tweet 2、tweet 6），替换为句号/逗号
- tweet 2「开始想另一件事」→「开始琢磨另一件事」
- tweet 2「这不只是 chatbot，这是」→「它已经不是 chatbot 了，这是」
- tweet 5 列表描述分隔符 — → ：
- 删 persuasive authority 痕迹
- 删 signposting
...
## post-2026-06-16-001：人形机器人正在进入「看谁拿到采购清单」的阶段

状态：draft
来源：inbox/2026-06.md#2026-06-16-10-48（优必选深度调研 V2）
首发平台：X
audience：AI builder + tech watchers + 关注具身智能产业化的从业者
是否升级长文：可联动（011 优必选双线战已 drafting，本条是支线早期信号）

### 一句话观点

人形机器人产业正在进入 AI 领域 2024 年的同一个拐点：从「能做什么」转向「谁先规模化、谁拿到采购清单」。

### 近似实现

- **大疆 DJI**：消费无人机「能飞」很早，胜负分水岭是 2015 年拿到农业/测绘等专业采购清单
- **Mobileye**：ADAS 视觉方案胜在「先上车」——早期绑定宝马/通用供应链
- **Figure 02 给 BMW 出货 ~500 台 + Tesla Optimus 内部 5K-10K 台 + Agility Digit 在 GXO ~500 台**：全球工业人形机器人当前规模对照
- **国内对标**：东风柳汽 20 台（2025 H1）+ 吉利/极氪 5G 智慧工厂 + 居然智家 500 台 + 天工行者 29.9 万/台定价
- 差异化：本条不只报数字，**重点是「阶段判断」** —— 把分散的 5 个信号收束成「采购清单拐点」框架

### X thread 草稿

**1/**
读了一份优必选深度调研。看完重新理解了一件事：人形机器人正在经历 AI 领域前两年的同一个拐点。

2022 年讨论大模型能做什么。2024 年变成「谁在用谁的 API」。现在人形机器人进入「谁先规模化、谁拿到采购清单」的阶段。🧵

**2/**
五个信号同时浮出水面：

① 东风柳汽 2025 H1 全球首次批量进车厂（20 台，不是 demo，是产线）
② Walker S2 2025-11-12 公告 8 亿+ 订单（不是意向，是真实交付合同）
③ 吉利/极氪 5G 智慧工厂多机协同
④ 居然智家 500 台 + 10000 台规划（不是车厂，是家居场景）
⑤ 天工行者 29.9 万/台定价远低于同类 50 万+
...
**4/**
但资本端估值差巨大：Figure $39B vs 优必选 HKD 480 亿 / $60 亿。

工业落地速度接近，估值差 6.5 倍。这中间差距来源是什么？是供应链能力的折价，还是叙事能力的差距？

**5/**
反面信号需要重视：

优必选 2020-2024 累计亏损 55+ 亿，2024 杠杆率 71.6%，计息借款 15.376 亿。

**6/**
2024-12-31 比亚迪/股东夏佐全减持 200 万股套现 1.12 亿港元（减持价 56.2 港元，比上市首日 90 港元跌 38%）。

大客户也是大股东，减持信号亮起。

**7/**
如果只看数字：1,079 台 → 2 万台（比亚迪目标），是 18.5 倍的跳跃。直觉是假的。
...
应用项：
- 删 em dash（"——" 全部换为句号或换行）
- 删"反直觉"/"耐人寻味"/"值得深思"等元评论
- 推 4"折现"/"折价"金融类比改为中性表述（"是中国供应链红利的估值差，还是叙事能力的折价？"改为"这中间差距来源是什么"）
- 推 7 保留数据冲击力，去修饰副词
- 推 8 不以金句收尾（"知道阶段比知道数字强"是判断不是金句）
- 全文无否定排比
...
## post-2026-06-16-002：3800 台 U1 预售，足够说明一件事

状态：draft
来源：inbox/2026-06.md#2026-06-16-10-48（优必选深度调研 V1 — U1 情感陪伴）
首发平台：X
audience：AI builder + tech watchers + 关注硬件 PMF 的从业者
是否升级长文：可联动（011 优必选双线战已 drafting，本条是 C 端 PMF 早期信号）

### 一句话观点

U1 3800 台预售不能回答「iPhone 时刻 vs 3D 电视」的问题。但它能回答另一个问题：愿意为「长着人类形态的陪伴设备」付钱的人，已经存在。

### 近似实现

- **Sony Aibo**：1999 年首发，2014 停产，2017 复活，2018 售罄，2019 年新一代 6 万台/3 个月。**陪伴机器人的「novelty 复活」先例**
- **Anki Vector / Cozmo**：2010s 桌面小机器人，2019 倒闭，留下「陪伴机器人的早期用户不是极客而是孤独的人」的教训
- **Jibo**：2014 众筹爆款，2018 倒闭，CSTO 评论「用户和它说话，但没人买第二台」
- **Loona by KEYi Tech**：2022 众筹破 1000 万，2024 持续在售
- **三星 Ballie**：2019 CES 概念，2024 仍未上市
- 差异化：本 thread 重点不在 U1 是不是好产品，**重点是「什么样的陪伴需求会留下来」**——Aibo/Jibo/Anki 三个失败/复活案例 + U1 3800 台数据综合判断

### X thread 草稿

**1/**
昨晚刷到一条：优必选 U1 情感陪伴机器人，30cm 小型人形，10 天预售 3800 台，知乎 101 万热度。

我第一反应是看定价。🧵

**2/**
定金 1000 万 / 3800 台 = 约 2600 元/台。

如果量产价 <1 万，那是大玩具。
如果 3-5 万，那是准伴侣。

定价策略本身就决定用户群。U1 没公布量产价，只收定金。这是个聪明的做法：先验证付费意愿，不承诺量价。
...
· Anki Vector/Cozmo：2019 倒闭。留下「陪伴机器人的早期用户不是极客而是孤独的人」的教训
...
3800 台不能回答 U1 是不是下一个 iPhone。但足够说明一件事：**愿意为「长着人类形态的陪伴设备」付钱的人，已经存在。**
...
应用项：
- 删 em dash（"——" 全部换为句号或换行）
- 推 2「聪明的做法」+「先验证付费意愿」AI 感（改为直接陈述「先收定金再决定量产价」）
- 推 5 全表格化对照（避免单点举例）
- 推 6 删「不约而同」/「耐人寻味」元评论
- 推 7 删 signposting「让我们意识到」改为直接陈述
- 推 8 收尾是判断不是金句
- 全文无否定排比
