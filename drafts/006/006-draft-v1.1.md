...
Fable 5 真实存在。Mythos 5 也真实存在。Stripe 5000 万行 Ruby 迁移、14 个蛋白靶点里产生 9 个候选、Project Glasswing、Hebbia 金融基准最高分，这些核心信息全都能被验证。唯一没有证据的是「Karpathy 加入 Anthropic」。

这件事本身就是一个信号。我们已经习惯把 AI agent 当作信息处理工具，但在真正的热点事件上，agent 的第一反应不是「查到了什么」，而是「我看不到，所以我判断它不存在」。正是因为第一轮被自己搭的 agent 骗了，我才决定不只看发布会，而是把英文社区、中文社区、benchmark、定价、社区争议全部跑一遍。
