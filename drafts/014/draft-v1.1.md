...
**什么是 harness？**——可以理解为「AI 编码工具的运行时框架」。Pi agent、OpenCode、Claude Code 都是 harness。它们负责工具调用、上下文管理、子任务编排、错误恢复——这些工程细节，决定 80% 的真实体验。
...
把上面的案例 + HN 主帖的高频配置 + Vicki Boykis 的模板组合起来，5 个可立即落地的部署栈。在列栈之前，先看清三组件怎么搭起来：

```
┌──────────────────────────────────────────────────────────────┐
│                Pi agent v0.79.9 (CLI / TUI)                   │
│  pi-ai (多 provider) + pi-agent-core (tool loop) +           │
│  pi-coding-agent (CLI) + pi-tui (终端 UI)                    │
│  5 种 install: curl / powershell / npm i -g / pnpm / bun    │
└──────────────────────┬───────────────────────────────────────┘
...
└──────────────────────┬───────────────────────────────────────┘
                       │ GGUF weights (~22 GB for Q4_K_M)
                       ▼
┌──────────────────────────────────────────────────────────────┐
│            Qwen 3.6 35B-A3B (Apache-2.0)                     │
│  35B 总 / 3B 激活 MoE / 256 routed experts (激活 8) / 1 shared│
│  40 层混合：10×(3×DeltaNet→MoE) + 1×(GatedAttn→MoE)         │
│  上下文：原生 262K → 1M via YaRN                              │
│  关键场景：SWE-bench Verified 73.4 / Terminal-Bench 51.5 第一│
└──────────────────────────────────────────────────────────────┘
```

三件事的分工：**Pi 决定你怎么用、llama.cpp 决定怎么跑、Qwen 决定能跑多好**。下面 5 个栈就是这三个维度在不同预算下的组合。
