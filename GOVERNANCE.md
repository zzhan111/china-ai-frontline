...
**`raw/` 的新增文件可以走 §3.5 豁免（express lane），但修改/删除/重命名已有 raw 文件没有豁免**。
新增不破坏不可变性——raw 的审计价值由「已有文件锁定」保证，
而新文件是真正的"追加"操作。但修改/删除/重命名已有的 raw 文件，
无论多小、多紧急，都不可豁免。
...
| 类型 | 路径范围 | 例 |
|---|---|---|
| 纯 typo / 标点 / 拼写修复 | 任意文件**除 `raw/` 中已有文件外** | 把"复盘"误打成"附盘"改回 |
| 链接修复 | 任意文件**除 `raw/` 中已有文件外** | 修复失效的相对路径 |
...
| 新增 raw 文件 | `raw/` 追加新文件（不是修改/删除/重命名已有文件） | 追加调研报告、session 记录等纯新增 raw |

**不可豁免的改动(黑名单,任何一条命中就必须走 PR)**:

- 任何对 `raw/` 中**已有文件**的修改、删除或重命名
...
- 任何**新增**文件（例外：新建当月 `inbox/YYYY-MM.md` 和新增 `raw/` 文件见上方白名单）
- 任何**删除**文件
- 任何**重命名**文件
